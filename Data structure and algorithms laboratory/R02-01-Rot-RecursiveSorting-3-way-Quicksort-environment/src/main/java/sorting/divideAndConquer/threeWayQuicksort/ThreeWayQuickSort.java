...
public class ThreeWayQuickSort<T extends Comparable<T>> extends
	...
	public void sort(T[] array, int leftIndex, int rightIndex) {
		if (leftIndex < rightIndex && array.length > 1) {
			int i = leftIndex, lowerIndex = leftIndex, greaterIndex = rightIndex;
			T n = array[leftIndex];
			
			while (i <= greaterIndex) {
				if (array[i].compareTo(n) < 0) {
					Util.swap(array, lowerIndex++, i++);
				} else if (array[i].compareTo(n) > 0) {
			...
			this.sort(array, leftIndex, lowerIndex - 1);
			this.sort(array, greaterIndex + 1, rightIndex);
		}
	}
