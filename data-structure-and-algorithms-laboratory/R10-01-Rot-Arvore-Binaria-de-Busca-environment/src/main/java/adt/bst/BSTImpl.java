...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	public boolean isBinarySearchTree() {
		...
		if (!this.isEmpty()) {
			result = this.isBinarySearchTree(this.root);
		}
	...
	private boolean isBinarySearchTree(BSTNode<T> node) {
		if (node.isEmpty()) {
			...
			if (!node.getLeft().isEmpty()) {
				isValidNode = node.getLeft().getData().compareTo(node.getData()) < 0;
			}
			
			if (!node.getRight().isEmpty()) {
				isValidNode = (isValidNode && node.getRight().getData().compareTo(node.getData()) > 0);
			}
			
			if (isValidNode) {
				return this.isBinarySearchTree((BSTNode<T>) node.getLeft()) && this.isBinarySearchTree((BSTNode<T>) node.getRight());
			} else {
				return false;
			}
		}
	...
	public T[] descendingOrder() {
		...
		this.descendingOrder(result, this.root, 0);
	...
	private int descendingOrder(T[] array, BSTNode<T> node, int index) {
		if (!node.isEmpty()) {
			index = this.descendingOrder(array, (BSTNode<T>) node.getRight(), index);
			array[index++] = node.getData();
			index = this.descendingOrder(array, (BSTNode<T>) node.getLeft(), index);
		}
		
		return index;
	}
	
	public boolean isEqualsBST(BST<T> bst) {
		return this.isEqualsBST(this.root, bst.getRoot());
	}
	
	private boolean isEqualsBST(BSTNode<T> node1, BTNode<T> node2) {
		if (node1.isEmpty() && node2.isEmpty()) {
			return true;
		} else {
			if (node1.equals(node2)) {
				return isEqualsBST((BSTNode<T>) node1.getLeft(), node2.getLeft()) 
						&& isEqualsBST((BSTNode<T>) node1.getRight(), node2.getRight());
			} else {
				return false;
			}
		}
	...
	public int leafs() {
		...
		if (!this.isEmpty()) {
			leafs = this.leafs(this.root);
		}
		
		return leafs;
	}
	
	public int leafs(BSTNode<T> node) {
		if (node.isEmpty()) {
			return 0;
		} else if (node.isLeaf()) {
			return 1;
		} else {
			return this.leafs((BSTNode<T>) node.getLeft()) + this.leafs((BSTNode<T>) node.getRight());
		}
	}
	...
	public boolean isDecendent (T d, T p) {
		...
		BSTNode<T> auxNode = this.search(p);
		
		if (auxNode != null && !auxNode.isEmpty()) {
			result = this.isDecendent(auxNode, d);
		}
		
		return result;
	}

	private boolean isDecendent(BSTNode<T> node, T d) {
		if (node.isEmpty()) {
			return false;
		} else {
			if (node.getData().equals(d)) {
				...
				if (node.getData().compareTo(d) > 0) {
					return this.isDecendent((BSTNode<T>) node.getLeft(), d);
				} else {
					return this.isDecendent((BSTNode<T>) node.getRight(), d);
				}
			}
		}
	}
	...
	public int distance(T x1, T x2) {
		int distance = -1;
		BSTNode<T> auxNode = this.search(x1);
		
		if (auxNode != null && !auxNode.isEmpty() && this.isDecendent(x2, x1)) {
			distance = this.distance(auxNode, x2);
		}
	...
	private int distance(BSTNode<T> node, T x2) {
		if (node.getData().equals(x2)) {
			return 0;
		} else {
			if (node.getData().compareTo(x2) > 0) {
				return 1 + this.distance((BSTNode<T>) node.getLeft(), x2);
			} else {
				return 1 + this.distance((BSTNode<T>) node.getRight(), x2);
			}
		}
	}
